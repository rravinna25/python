...
def  fact(n):
	if  n > 0:
		return  n * fact(n - 1)   #  Continues  recursion  as  long  as  n > 0
	else:
		return   1  #  Suspends  recursion  as  soon   as  'n'  is  0
'''
fact(4) = 4 * fact(3)
            = 4 * 3 * fact(2)
            = 4 * 3 * 2 * fact(1)
			...
			= 24           
'''
n = int(input('Enter  any  +ve  integer  or  0  :  '))  #  5
print(F'{n}!  =  {fact(n)}')  #  5!  =  120
...
3) def  fact(n):
		    return  n * fact(n - 1)
    What  is  the  result  of  fact(4)  without  if  statement  ?  ---> 4 * fact(3)
																										 =  4 * 3 * fact(2)
																										 =  4 * 3 * 2 * fact(1)
																										 =  4 * 3 * 2 * 1 * fact(0)
																										 =  4 * 3 * 2 * 1 * 0 * fact(-1)
...
8) In  other  words,  result  is  always  1  for  every  input

9) Can  if  conidition  be  changed  to  n == 0  ?  --->  Yes  but  the  two  return  statements  need  to  be  interchanged
																			      Eg:  if  n  == 0:
																							 return   1
																						 else:
																							  return  n * fact(n - 1)
