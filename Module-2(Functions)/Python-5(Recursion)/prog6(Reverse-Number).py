...
def  rev(n):
	if  n > 0:
		return  n % 10 * 10 ** (len(str(n)) - 1) + rev(n // 10)  #  Continue  recursion  as  long  as  n > 0
	else:
		return  0  #  Suspend  recursion  as  soon  as  n  is  0
'''
1) rev(946) = 946 % 10 * 10 ** 2 + rev(946 // 10)
                  = 600 + rev(94)
				  = 600 + 94 % 10 * 10 ** 1 + rev(94 // 10)
				  = 600 + 40 + rev(9)
				  = 600 + 40 + 9 % 10 * 10 ** 0 + rev(9 // 10)
				  = 600 + 40 + 9 + rev(0)
				  = 600 + 40 + 9 + 0
				  = 649
				  
2) Can  if  condition  be  changed  to  n  ==  0 ?  --->  Yes  but  the  two  return  statements  need  to  be  interchanged				  
'''
